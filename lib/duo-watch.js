...
function watch(root, opts) {
  ...
  return watching[root] = new Watch(root, opts);
}
...
function Watch(root, opts) {
  if (!(this instanceof Watch)) return new Watch(root, opts);
  assert(root, 'must pass a "root" into watch');

  var self = this;

  opts = opts || {};
  opts.installTo = opts.installTo || 'components';
  this.path = join(root, opts.installTo, 'duo.json');
